...
class NotificationEngine:
    ...
    def subscribe(self, key: NotificationKey, callback: Optional[F] = None) -> Union[F, Callable[[F], F]]:
        ...
        """
        def wrapper(callback: Callable[[NotificationLog], None]) -> Callable[[NotificationLog], None]:
                ...
                try:
                    ...
                    for callback in callbacks:
                        try:
                            ...
                            logger.error(f"Error in executing callback for key {notification.key}: {e}")
...
async def example():
    ...
    async def handler(notification):
        print(f"Handler 1: {notification}")

    async def handler2(notification):
        print(f"Handler 2: {notification}")

    from sqlmodel import SQLModel
    from datetime import datetime
    class TestModel(SQLModel, table=False):
        id: int
        name: str

    log1 = NotificationLog(NotificationKey("test", "test"), TestModel(id=1, name="test"), datetime.now())
    log2 = NotificationLog(NotificationKey("test", "test"), TestModel(id=2, name="test"), datetime.now())
    await engine.notify(log1)
    await engine.notify(log2)


    engine.subscribe(NotificationKey("test", "test"), handler)
    await engine.start()
